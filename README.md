...
Formal validity is model-relative. A plan passing VAL is valid with respect to the supplied PDDL domain and problem. It does not prove real-world feasibility.

VAL and the Python symbolic verifier have different roles. For LLM-generated plans, VAL provides the final formal validity judgement. The Python symbolic verifier provides domain-specific diagnosis and failure localisation.

Pure PDDL uses a combined success criterion. A Pure PDDL run is successful only when Fast Downward solves the task, the symbolic verifier succeeds, and VAL returns valid.

Hybrid refinement uses complete-plan regeneration. The system does not perform local plan patching.

Fast Downward is not used for Hybrid repair. Fast Downward is the planner for the Pure PDDL condition.

The project does not implement LLM-guided classical search-space reduction. LLM-generated search constraints, intermediate subgoals, or heuristics for guiding Fast Downward remain a possible future extension rather than an implemented experiment.

Recorded VAL timing is not end-to-end runtime. The total_val_runtime_seconds and average_val_runtime_seconds fields describe VAL validation timing, not the complete computational cost of LLM generation, Fast Downward search, symbolic diagnosis, and refinement.

The repeated formal LLM runs use temperature 0.0 with fixed configurations. They are separate executions, but they should not be interpreted as highly variable independent stochastic samples.

The Easy/Medium/Hard labels are within-domain task categories. They are not a common numerical difficulty scale across all three domains.

inferred_root_cause is an auxiliary field. It includes domain-specific heuristics and is not a universal failure taxonomy.
